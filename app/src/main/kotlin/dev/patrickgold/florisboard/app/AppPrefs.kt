...
abstract class FlorisPreferenceModel : PreferenceModel() {
    ...
    inner class Smartbar {
        ...
        val aiToolbarEnabled = boolean(
            key = "smartbar__ai_toolbar_enabled",
            default = true,
        )
        val aiApiKey = string(
            key = "smartbar__ai_api_key",
            default = "",
        )
        val aiApiBaseUrl = string(
            key = "smartbar__ai_api_base_url",
            default = "https://api.florisboard.ai/v1",
        )
        val aiUseFakeData = boolean(
            key = "smartbar__ai_use_fake_data",
            default = true,
        )
    }
